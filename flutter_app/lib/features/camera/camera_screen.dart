...
const _kStartupInitDelay = Duration(milliseconds: 120);
const _kStartupTransitionTail = Duration(milliseconds: 260);
const _kLifecycleResumeTail = Duration(milliseconds: 90);
...
class _CameraScreenState extends ConsumerState<CameraScreen>
  ...
  void initState() {
    ...
    // ── 启动画面：第一帧渲染后立即显示 App Icon 过渡覆盖层 ──
    // 与切换相机时的覆盖层完全相同，用户看到 App Icon 而非空白黑屏
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) setState(() => _showTransition = true);
    });

    WidgetsBinding.instance.addPostFrameCallback((_) async {
      // 请求权限（App Icon 画面遮住，用户不会看到黑屏）
      ...
      // 修复：检测到「本次是首次授权」时，再执行一次 initCamera 确保预览正常输出。
  ...
  Future<void> _showCameraTransition(FutureOr<void> Function() action,
      {Duration minVisible = const Duration(milliseconds: 120)}) async {
    ...
    await Future.delayed(_kActionTransitionLead);
  ...
  void _enqueueSceneHint(String nextKey) {
    ...
    final elapsed = now.difference(_sceneHintLastSwitchAt);
    final remainingVisible = _kSceneHintMinVisible - elapsed;
    final delay = remainingVisible > _kSceneHintCandidateHold
        ? remainingVisible
        : _kSceneHintCandidateHold;

    _pendingSceneHintKey = nextKey;
    _sceneHintCommitTimer?.cancel();
    _sceneHintCommitTimer = Timer(delay, () {
      if (!mounted || _pendingSceneHintKey != nextKey) return;
      setState(() {
        _sceneHintKey = nextKey;
        _pendingSceneHintKey = null;
  ...
  String _sceneHintLabel(S s) {
    switch (_sceneHintKey) {
      case 'sceneHintFisheye':
        return s.sceneHintFisheye;
      case 'sceneHintMacro':
        return s.sceneHintMacro;
      case 'sceneHintPortrait':
        return s.sceneHintPortrait;
      case 'sceneHintLowLight':
        return s.sceneHintLowLight;
      case 'sceneHintBacklit':
        return s.sceneHintBacklit;
      case 'sceneHintIndoor':
        return s.sceneHintIndoor;
      case 'sceneHintOutdoor':
        return s.sceneHintOutdoor;
      case 'sceneHintHighRes':
        return s.sceneHintHighRes;
      default:
        return s.sceneHintBalanced;
    }
  }
